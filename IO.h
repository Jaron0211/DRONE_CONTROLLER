...
void POSITION() {
  ...
  if (ARM) {
    ...
    if (pos_value[2] > 1200) {
      pos_control(pos_value[2],pos_value[0], pos_value[1], pos_value[3], 0);
      M1_VAL = constrain((HEI_PID_OUTPUT + PITCH_PID_OUTPUT - ROLL_PID_OUTPUT + YAW_PID_OUTPUT), START_SPEED, MAX_SPEED);
      M2_VAL = constrain((HEI_PID_OUTPUT - PITCH_PID_OUTPUT + ROLL_PID_OUTPUT + YAW_PID_OUTPUT), START_SPEED, MAX_SPEED);
      M3_VAL = constrain((HEI_PID_OUTPUT + PITCH_PID_OUTPUT + ROLL_PID_OUTPUT - YAW_PID_OUTPUT), START_SPEED, MAX_SPEED);
      M4_VAL = constrain((HEI_PID_OUTPUT - PITCH_PID_OUTPUT - ROLL_PID_OUTPUT - YAW_PID_OUTPUT), START_SPEED, MAX_SPEED);
    }
    else {
      pos_control(1500,0, 0, 0, 1);
      M1_VAL = START_SPEED;
      M2_VAL = START_SPEED;
      M3_VAL = START_SPEED;
      M4_VAL = START_SPEED;
